...
Promise.all([
  ...
  for (const [word, data] of Object.entries(dictionary)) {
    ...
    const meaningTexts = Array.isArray(data.meaning)
      ? data.meaning.map(m => removeAnnotations(m.text || ""))
      : data.meaning ? [removeAnnotations(String(data.meaning))] : [];

    data._normMeaningList = meaningTexts.map(t => normalizeForSearch(t));
...
function performSearch() {
  ...
  if (searchMode === "tag") {
    ...
    const primaryResults = Object.keys(dictionary).filter(word => {
      ...
      const matchMeaning = (() => {
        const list = data._normMeaningList || [];

        if (searchMode === "exact") {
          return list.some(t => t === normalizedSearch);
        }
        if (searchMode === "prefix") {
          return list.some(t => t.startsWith(normalizedSearch));
        }
        return list.some(t => t.includes(normalizedSearch));
      })();
